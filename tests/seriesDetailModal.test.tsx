...
import { SeriesDetailModal } from '../src/components/modals/SeriesDetailModal';
import * as useDetailsModule from '../src/api/useDetails';
...
vi.mock('../src/api/tmdb', () => tmdb);
const upcoming = vi.hoisted(() => ({ useUpcomingReleases: vi.fn() }));
vi.mock('../src/api/useUpcomingReleases', () => upcoming);
...
describe('SeriesDetailModal Component', () => {
  const defaultProps = {
    seriesId: 'series-123',
  ...
  beforeEach(() => {
    ...
    useAuthStore.setState({ profiles: [], runtimes: {}, credentials: null, userInfo: null, serverInfo: null });
  ...
  it('opens the requested season and identifies the requested episode', () => {
    ...
    renderWithRouter(
      <SeriesDetailModal {...defaultProps} initialSeasonNumber={2} initialEpisodeNumber={1} />,
    );
