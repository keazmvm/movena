...
describe('M3U channel table model', () => {
  it('recovers safe defaults from corrupt or unrecognized persisted filters', () => {
    ...
    expect(readM3uTableFilters({ getItem: (key) => key === M3U_TABLE_FILTER_STORAGE_KEY
      ? JSON.stringify({ selectedGroup: 42, mediaTypeFilter: 'radio', healthFilter: 'checking', sortBy: 'random' })
      : null })).toEqual(DEFAULT_M3U_TABLE_FILTERS)
  })

  it('counts stable alphabetized groups and assigns blank groups to General', () => {
    expect(collectM3uGroupStats([
      entry('one', { groupTitle: 'Sports' }),
      entry('two'),
      entry('three', { groupTitle: 'Sports' }),
    ])).toEqual([{ name: 'General', count: 1 }, { name: 'Sports', count: 2 }])
  })
  ...
  it('combines group, media, health, and metadata search filters', () => {
    ...
    expect(filterAndSortM3uEntries(entries, {
  ...
  it('does not mutate the source array while sorting', () => {
    ...
    expect(filterAndSortM3uEntries(entries, {}, { ...DEFAULT_M3U_TABLE_FILTERS, sortBy: 'name-asc' })
      .map(({ id }) => id)).toEqual(['alpha', 'beta'])
