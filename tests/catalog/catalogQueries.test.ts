...
const xc = vi.hoisted(() => ({
  ...
  getVodStreams: vi.fn(),
  getStreamUrl: vi.fn((credentials: { url: string }, type: string, id: string | number, extension?: string) =>
...
const xtreamProfile = {
  id: 'xtream-one', kind: 'xtream', name: 'Provider', locationLabel: 'provider.test', username: 'alice',
  userInfo: { auth: 1 }, serverInfo: {}, createdAt: 1, updatedAt: 1,
};

function sources(overrides: Partial<EnabledSourcesSnapshot> = {}): EnabledSourcesSnapshot {
  const xtreamSource = {
    id: 'xtream-one', profile: xtreamProfile, runtime: { credentials, status: 'ready', error: null, revision: 1 },
    credentials, queryScope: 'xtream-scope', isAvailable: true,
  };
  return {
...
describe('source-scoped catalog and category queries', () => {
  ...
  it('maps Xtream and M3U live items without leaking source identity', async () => {
    ...
    const m3uSource = {
      id: 'm3u-one', profile: { id: 'm3u-one', name: 'Local', kind: 'm3u' },
      runtime: { playlist, connection: null, status: 'ready', error: null, revision: 1 },
      queryScope: 'm3u-scope', isAvailable: true,
    } as any;

    const result = await runQuery<any>(catalogQueryOptions('live', sources({
      availableM3uSources: [m3uSource],
      m3uSources: [m3uSource],
    })));

    expect(result).toEqual(expect.arrayContaining([
      expect.objectContaining({ id: 'xtream-one:live:42', sourceId: 'xtream-one', categoryId: 'xtream-one:category:7', radio: true }),
      expect.objectContaining({ id: expect.stringMatching(/^m3u-/), sourceId: 'm3u-one', streamUrl: 'https://m3u.test/news.ts' }),
    ]));
  ...
  it('shares one folded live result between observers of the same catalog', () => {
    const catalog = [
      { id: 'one', title: 'News HD', posterUrl: '', type: 'live' as const, streamUrl: 'https://one.test' },
      { id: 'two', title: 'News HD', posterUrl: '', type: 'live' as const, streamUrl: 'https://two.test' },
    ];
  ...
  it('keeps successful providers when another enabled provider fails, but errors when none work', async () => {
    const second = {
      id: 'xtream-two', profile: { ...xtreamProfile, id: 'xtream-two', name: 'Backup' },
      runtime: { credentials: { ...credentials, sourceId: 'xtream-two', url: 'https://backup.test' }, status: 'ready', error: null, revision: 1 },
      credentials: { ...credentials, sourceId: 'xtream-two', url: 'https://backup.test' }, queryScope: 'backup', isAvailable: true,
    } as any;
    xc.getVodStreams
      .mockRejectedValueOnce(new Error('primary offline'))
      .mockResolvedValueOnce([{ stream_id: 8, name: 'Backup Movie', stream_icon: '', category_id: '3', container_extension: 'mkv' }]);

    const result = await runQuery<any>(catalogQueryOptions('vod', sources({
    ...
    await expect(runQuery<any>(catalogQueryOptions('vod', sources()))).rejects.toThrow('all offline');
  });
  ...
  it('retains every provider name and technical failure when all catalog sources fail', async () => {
    ...
    await expect(runQuery<any>(catalogQueryOptions('vod', sources({
  ...
  it('merges provider and playlist categories with source-scoped provider ids', async () => {
    xc.getLiveCategories.mockResolvedValue([{ category_id: '7', category_name: 'DE | News', parent_id: 0 }]);
    const playlist = parseM3u(
    ...
    const result = await runQuery<any>(categoriesQueryOptions('live', sources({
      availableM3uSources: [m3uSource],
      m3uSources: [m3uSource],
    })));
    ...
    expect(result).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ category_id: expect.stringMatching(/^m3u-category-/), category_name: 'Local News' }),
    ]));
  });
  ...
  it('fails category loading only when every available provider fails', async () => {
    ...
    await expect(runQuery<any>(categoriesQueryOptions('series', sources())))
      .rejects.toThrow('series categories offline');

    xc.getSeriesCategories.mockResolvedValue([{ category_id: '9', category_name: 'Drama', parent_id: 0 }]);
    await expect(runQuery<any>(categoriesQueryOptions('series', sources())))
      .resolves.toEqual([{ category_id: 'xtream-one:category:9', category_name: 'Drama', parent_id: 0 }]);
  });
});
