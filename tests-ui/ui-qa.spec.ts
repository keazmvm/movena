...
import { UI_LANGUAGES } from '../src/i18nConfig';
...
async function readUndersizedTargets(page: import('@playwright/test').Page) {
  return page.locator('button, a, input, [role="button"], [role="menuitem"], [role="tab"]').evaluateAll((elements, minimum) => elements
    .filter((element) => {
...
async function readClippedDialogs(page: import('@playwright/test').Page) {
  return page.locator('[role="dialog"], [role="alertdialog"]').evaluateAll((dialogs) => dialogs
    .filter((dialog) => {
...
for (const scenario of PRODUCTION_UI_QA_SCENARIOS) {
  for (const theme of scenario.themes) {
    test(`${scenario.id} (${theme}) satisfies production geometry and accessibility contracts`, async ({ page }, testInfo) => {
      const errors: string[] = [];
      page.on('console', (message) => {
        if (message.type() === 'error') errors.push(message.text());
      });
      ...
      for (const viewport of [
        ...
        await expect(page.locator('[data-ui-qa-scenario]')).toHaveAttribute('data-ui-qa-scenario', scenario.id);
        await page.waitForTimeout(200);

        const overflow = await readHorizontalOverflow(page);
        expect(overflow.document, `${viewport.width}×${viewport.height} document horizontal overflow`).toBeLessThanOrEqual(1);
        expect(overflow.body, `${viewport.width}×${viewport.height} body horizontal overflow`).toBeLessThanOrEqual(1);
        expect(await readUndersizedTargets(page), `${viewport.width}×${viewport.height} targets smaller than 24×24`).toEqual([]);
        expect(await readClippedDialogs(page), `${viewport.width}×${viewport.height} clipped dialogs`).toEqual([]);

        for (const layer of scenario.expectedLayers) {
          await expect(page.locator(`[data-ui-layer="${layer}"]`).first(), `${scenario.id} exposes ${layer} layer marker`).toBeVisible();
        }
...
for (const locale of UI_LANGUAGES) {
  test(`critical production geometry remains stable in ${locale}`, async ({ page }) => {
    ...
    const criticalScenarios = PRODUCTION_UI_QA_SCENARIOS.filter((scenario) => [
      'hero', 'live-epg', 'series-details', 'm3u-editor', 'settings-general', 'player-series',
    ].includes(scenario.id));

    for (const scenario of criticalScenarios) {
      await page.setViewportSize({ width: 960, height: 600 });
      await page.goto(withHarnessOptions(scenario.route, locale, scenario.fixtureSetup === 'native-player' ? 'dark' : 'light'));
      await expect(page.locator('[data-ui-qa-scenario]')).toHaveAttribute('data-ui-qa-scenario', scenario.id);
      const overflow = await readHorizontalOverflow(page);
      expect(overflow.document, `${scenario.id} ${locale} minimum-width overflow`).toBeLessThanOrEqual(1);
      expect(await readClippedDialogs(page), `${scenario.id} ${locale} clipped dialogs`).toEqual([]);
    }
  });
}

test('live catalog header controls never overlap through intermediate workspace widths', async ({ page }, testInfo) => {
  await page.clock.setFixedTime(UI_QA_FIXED_TIME);
  ...
  for (const viewport of [
    ...
    await expect(page.locator('[data-ui-qa-scenario]')).toHaveAttribute('data-ui-qa-scenario', 'live-tv');
    ...
    const boxes = Object.fromEntries(await Promise.all(Object.entries(controls).map(async ([name, locator]) => {
    ...
    for (const [name, box] of Object.entries(boxes)) {
      expect(box.x, `${viewport.name}: ${name} starts inside the viewport`).toBeGreaterThanOrEqual(0);
      expect(box.y, `${viewport.name}: ${name} starts inside the viewport`).toBeGreaterThanOrEqual(0);
      expect(box.x + box.width, `${viewport.name}: ${name} ends inside the viewport width`).toBeLessThanOrEqual(viewport.width + 1);
      expect(box.y + box.height, `${viewport.name}: ${name} ends inside the viewport height`).toBeLessThanOrEqual(viewport.height + 1);
    }
    ...
    for (const [left, right] of [
      ...
      expect(overlaps(boxes[left], boxes[right]), `${viewport.name}: ${left} must not overlap ${right}`).toBe(false);
    }

    const overflow = await readHorizontalOverflow(page);
    expect(overflow.document, `${viewport.name} document horizontal overflow`).toBeLessThanOrEqual(1);
    expect(overflow.body, `${viewport.name} body horizontal overflow`).toBeLessThanOrEqual(1);

    if (VISUAL_COMPARISON) {
      await expect(page).toHaveScreenshot(`live-tv-header-${viewport.name}-${process.platform}.png`, { fullPage: true });
    } else {
...
test('the 960×600 logical minimum remains unchanged at 200% device scale', async ({ browser }) => {
  ...
  try {
    for (const scenarioId of ['live-tv', 'live-epg', 'series-details', 'm3u-editor', 'settings-general', 'player-series']) {
      ...
      await page.goto(withHarnessOptions(scenario!.route, 'en', scenario!.fixtureSetup === 'native-player' ? 'dark' : 'light'));
      await expect(page.locator('[data-ui-qa-scenario]')).toHaveAttribute('data-ui-qa-scenario', scenarioId);
