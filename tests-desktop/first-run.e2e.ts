...
describe('packaged desktop first run', () => {
  after(async () => {
    const sourceIds = await browser.execute(() => {
      try {
        const profiles = JSON.parse(localStorage.getItem('movena-source-profiles-v1') ?? '[]') as Array<{ id?: unknown }>
  ...
  it('crosses the real React/Tauri boundary and completes onboarding', async () => {
    const cleared = await browser.tauri.execute(({ core }) => core.invoke('app_data_clear', { sourceIds: [] }))
