...
export async function listM3uVersions(sourceId: string): Promise<M3uVersionRecord[]> {
  ...
  try {
    ...
    return parsed.flatMap((candidate) => {
      ...
      if (typeof record.id !== 'string' || record.sourceId !== sourceId || typeof record.createdAt !== 'number'
        || !Number.isFinite(record.createdAt) || typeof record.label !== 'string'
        || typeof record.entryCount !== 'number' || !Number.isFinite(record.entryCount)
        || typeof record.content !== 'string') return [];
      return [{
        id: record.id,
        sourceId,
    ...
    }).sort((a, b) => b.createdAt - a.createdAt).slice(0, MAX_VERSIONS_PER_SOURCE);
  } catch {
    return [];
  }
}

export async function saveM3uVersion(input: Omit<M3uVersionRecord, 'id' | 'createdAt'>): Promise<M3uVersionRecord> {
  const createdAt = Date.now();
  const record: M3uVersionRecord = { ...input, createdAt, id: makeId(input.sourceId, createdAt) };
  const versions = [record, ...(await listM3uVersions(input.sourceId)).filter((version) => version.id !== record.id)]
...
export async function deleteM3uVersion(id: string): Promise<void> {
  ...
  if (versions.length > 0) await storeM3uEditorState(HISTORY_NAMESPACE, sourceId, JSON.stringify(versions));
