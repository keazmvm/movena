...
export type M3uTableHealthFilter = 'all' | 'online' | 'offline' | 'unauthorized' | 'timeout' | 'untested'
...
export function readM3uTableFilters(storage: Pick<Storage, 'getItem'> = localStorage): M3uTableFilters {
  try {
    const value = JSON.parse(storage.getItem(M3U_TABLE_FILTER_STORAGE_KEY) || '{}') as Record<string, unknown>
    return {
      searchQuery: '',
...
export function collectM3uGroupStats(entries: readonly M3uEntry[]): Array<{ name: string; count: number }> {
...
export function filterAndSortM3uEntries(
  ...
  if (query) {
    result = result.filter((entry) => [entry.title, entry.url, entry.tvgId, entry.groupTitle]
      .some((value) => value?.toLocaleLowerCase().includes(query)))
  }
  const compare = filters.sortBy === 'name-asc'
    ...
    : filters.sortBy === 'name-desc'
      ...
      : filters.sortBy === 'chno'
        ? (left: M3uEntry, right: M3uEntry) => (Number(left.channelNumber) || Infinity) - (Number(right.channelNumber) || Infinity)
        : filters.sortBy === 'type'
