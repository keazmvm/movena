...
export type PlaylistSnapshot = Pick<M3uPlaylist, 'name' | 'epgUrls' | 'entries' | 'warnings' | 'extraHeaderAttributes' | 'extraDirectives'>;
...
export const emptyRawEditorViewState = (): M3uRawEditorViewState => ({ selectionStart: 0, selectionEnd: 0, scrollTop: 0, scrollLeft: 0 });
...
export function playlistSnapshot(playlist: M3uPlaylist): PlaylistSnapshot {
  return {
    ...
    ...(playlist.extraHeaderAttributes !== undefined ? { extraHeaderAttributes: playlist.extraHeaderAttributes } : {}),
    ...(playlist.extraDirectives !== undefined ? { extraDirectives: playlist.extraDirectives } : {}),
  };
}
