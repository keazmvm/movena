...
export function M3uBatchToolsModal({
  entries,
  selectedIds,
  isOpen,
  onClose,
  onApply,
}: M3uBatchToolsModalProps) {
  ...
  const [targetScope, setTargetScope] = useState<'all' | 'selected'>(selectedIds.size > 0 ? 'selected' : 'all');
  ...
  const saveCurrentRule = () => {
    ...
    const preset: M3uTransformPreset = activeTab === 'clean'
      ...
      >
        ...
        <div className={styles.modalBody}>
          ...
          {selectedIds.size > 0 && (
            <div className={styles.formGroup}>
              ...
              <SegmentedControl
                options={[
                  { value: 'selected', label: t('Selected ({count})', { count: number(selectedIds.size) }) },
            ...
            <>
              ...
              <div className={styles.formGroup}>
                <span className={styles.formLabel}>
                  {t('Preview Changes ({count} channels will be modified)', { count: number(cleanPreview.length) })}
                </span>
                {cleanPreview.length > 0 ? (
                  <div className={styles.diffPreviewList}>
                    ...
                    {cleanPreview.length > 50 && (
                      <span style={{ fontSize: 'var(--font-size-micro)', color: 'var(--text-muted)', textAlign: 'center' }}>
          ...
          {activeTab === 'replace' && (
            <>
              <div className={styles.formRow}>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel} htmlFor="m3u-replace-field">{t('Target Field')}</label>
                  <Select
                    value={findField}
                    options={[
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel} htmlFor="m3u-find-text">{t('Find Text')}</label>
                <input
                  id="m3u-find-text"
                  type="text"
                  className={`uiField`}
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel} htmlFor="m3u-replace-text">{t('Replace With')}</label>
                <input
                  id="m3u-replace-text"
                  type="text"
                  className={`uiField`}
          ...
          {activeTab === 'renumber' && (
            <>
              <div className={styles.formGroup}>
                <label className={styles.formLabel} htmlFor="m3u-renumber-start">{t('Starting Number')}</label>
                <input
                ...
                />
                <span style={{ fontSize: 'var(--font-size-small)', color: 'var(--text-muted)' }}>
                  {t('Renumber {count} channels sequentially in their current order.', { count: number(scopeEntries.length) })}
                </span>
              </div>
            </>
          )}
          ...
          {(activeTab === 'clean' || activeTab === 'replace') && (
            <div className={styles.savedRuleComposer}>
              <label className={styles.formGroup} htmlFor="m3u-rule-name">
                ...
                <input id="m3u-rule-name" className="uiField" value={presetName} onChange={(event) => setPresetName(event.target.value)} placeholder={t('Rule name')} />
              </label>
              <Button variant="ghost" size="sm" type="button" onClick={saveCurrentRule} disabled={!presetName.trim() || (activeTab === 'replace' && !findText)}>
              ...
              {presets.map((preset) => {
                ...
                return (
                  <div key={preset.id} className={styles.presetRow}>
                    <div><strong>{preset.name}</strong><span>{t(preset.kind === 'clean' ? 'Title cleanup' : 'Find and replace')} · {number(preview.count)} {t('matches')}</span></div>
                    <Button variant="primary" size="sm" type="button" onClick={() => applyPreset(preset)} disabled={preview.count === 0}><Play size={13} /> {t('Apply')}</Button>
                    <IconButton size="sm" type="button" onClick={() => deletePreset(preset.id)} aria-label={t('Delete saved rule')}><Trash2 size={13} /></IconButton>
                  </div>
                );
              })}
              {presets.length === 0 && <p className={styles.emptyNotice}>{t('No saved rules yet. Configure a title cleanup or replacement and save it for reuse.')}</p>}
            </div>
          )}
        </div>
        ...
        <div className={styles.modalFooter}>
          ...
          {activeTab === 'clean' && (
            ...
            >
              <Sparkles size={14} /> {t('Apply Clean ({count})', { count: number(cleanPreview.length) })}
            </Button>
          )}
          {activeTab === 'replace' && (
            <Button
            ...
            >
              <Replace size={14} /> {t('Replace All ({count})', { count: number(replacePreview.count) })}
            </Button>
          )}
          {activeTab === 'renumber' && (
