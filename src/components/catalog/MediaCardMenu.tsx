...
export function MediaCardMenu({ item, currentCollectionId, onPlay, onViewDetails }: MediaCardMenuProps) {
  const { t } = useI18n();
  ...
  const isFavorite = useLibraryStore((state) => state.favorites.some((entry) => entry.id === item.id)) || Boolean(item.isFavorite);
  const isWatched = useLibraryStore((state) => state.watched.includes(item.id)) || Boolean(item.isWatched);
  ...
  return (
    <>
      ...
      {isOpen && position && createPortal(
        ...
        >
          {item.type === 'live' ? (
            <button type="button" className={styles.menuItem} onClick={(event) => run(event, onPlay)}>
              <Play size={14} /> <span>{t('Tune Channel')}</span>
            </button>
          ) : (
            <button type="button" className={styles.menuItem} onClick={(event) => run(event, onViewDetails)}>
              <Info size={14} /> <span>{t('View Details')}</span>
            </button>
          )}
          {(item.type === 'vod' || item.type === 'series') && item.streamUrl && (
            <button type="button" className={styles.menuItem} onClick={(event) => run(event, () => { void downloadMediaItem(item); })}>
          ...
          <button type="button" className={styles.menuItem} onClick={(event) => run(event, () => useLibraryStore.getState().toggleWatched(item.id))}>
            {isWatched ? <CheckCircle size={14} className={styles.menuItemActive} /> : <Circle size={14} />}
          ...
          <div className={styles.menuDivider} />
          <button type="button" className={styles.menuItem} onClick={(event) => run(event, () => {
            ...
            <Copy size={14} /> <span>{t(item.streamUrl ? 'Copy Stream Link' : 'Copy Title')}</span>
          </button>

          {currentCollectionId && (
            <button type="button" className={`${styles.menuItem} ${styles.menuItemDanger}`} onClick={(event) => run(event, () => useLibraryStore.getState().removeFromCollection(currentCollectionId, item.id))}>
              <FolderMinus size={14} /> <span>{t('Remove from Collection')}</span>
            </button>
          )}
          {inHistory && (
            <button type="button" className={`${styles.menuItem} ${styles.menuItemDanger}`} onClick={(event) => run(event, () => useLibraryStore.getState().removeFromHistory(item.id))}>
